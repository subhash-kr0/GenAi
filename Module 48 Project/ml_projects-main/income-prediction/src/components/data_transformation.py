...
class DataTransformation:
    ...
    def get_merged_batch_data(valid_data_dir: str) -> pd.DataFrame:
        ...
        try:
            ...
            merged_data = merged_data.drop(columns="education", axis=1)  # removing the column as it doesn't contribute to prediction.
    ...
    def get_data_transformer_object(self, dataframe: pd.DataFrame):
        try:
            ...
            cat_features, cont_features, disc_features = self.utils.identify_feature_types(dataframe)
            cat_features = [column for column in cat_features if column not in numeric_features]
